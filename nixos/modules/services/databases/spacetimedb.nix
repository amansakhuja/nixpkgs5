...
in
{
  options.services.spacetimedb = {
    ...
    openFirewall = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to open the firewall for the port in {option}`services.spacetimedb.server.port`.";
    ...
    server = {
      ...
      inMemory = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "If specified the database will run entirely in memory. After the process exits all data will be lost.";
      };
    };
    ...
    nginx = {
      ...
      restrictPublish = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = ''
          Whether to restrict the publish database route to local connections.
        '';
      };
    };
  };
