...
in {
  options = {
    services.libretranslate = {
      enable = lib.mkEnableOption (lib.mdDoc "LibreTranslate service");
      package = lib.mkPackageOption pkgs "libretranslate" {
        default = [ "libretranslate" ];
        extraDescription = lib.mdDoc "LibreTranslate package to use.";
      };
      user = lib.mkOption {
        type = lib.types.str;
        default = "libretranslate";
        description = lib.mdDoc "User account under which libretranslate runs.";
      };
      group = lib.mkOption {
        type = lib.types.str;
        default = "libretranslate";
        description = lib.mdDoc "Group account under which libretranslate runs.";
      };
      host = lib.mkOption {
        description = lib.mdDoc "The address the application should listen on.";
        type = lib.types.str;
        default = "127.0.0.1";
      };
      port = lib.mkOption {
        type = lib.types.port;
        default = 5000;
        description = lib.mdDoc "The the application should listen on.";
      };
      dataDir = lib.mkOption {
        type = lib.types.path;
        default = "/var/lib/libretranslate";
        example = "/srv/data/libretranslate";
        description = lib.mdDoc "The data directory.";
      };
      threads = lib.mkOption {
        type = lib.types.nullOr lib.types.ints.positive;
        default = null;
        example = 8;
        description = lib.mdDoc "Set number of threads.";
      };
      enableApiKeys = lib.mkOption {
        type = lib.types.bool;
        default = false;
        example = true;
        description = lib.mdDoc "Whether to enable the API keys database.";
      };
      disableWebUI = lib.mkOption {
        type = lib.types.bool;
        default = false;
        example = true;
        description = lib.mdDoc "Whether to disable the Web UI.";
      };
      ...
      configureNginx = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = lib.mdDoc "Configure nginx as a reverse proxy for LibreTranslate.";
      };
      extraArgs = lib.mkOption {
        type = with lib.types; attrsOf (nullOr (oneOf [ bool str int (listOf (oneOf [ bool str int ])) ]));
        default = { };
        example = {
          debug = true;
        ...
        description = lib.mdDoc "Extra arguments passed to the LibreTranslate.";
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.libretranslate = {
      ...
      serviceConfig = lib.mkMerge [{
        Type = "simple";
        ExecStart = ''
          ${cfg.package}/bin/libretranslate ${lib.cli.toGNUCommandLineShell { }
            (cfg.extraArgs // {
        ...
        SystemCallFilter = [ "~@cpu-emulation @debug @ipc @keyring @mount @obsolete @privileged @setuid" ];
      }
